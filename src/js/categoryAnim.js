...
export function categoryAnim() {
    ...
    if (window.innerWidth >= 1024) {
        renderCards = async function (category) {
            ...
            tl.fromTo(
                ".cardWidth",
                { x: function (idx) { return idx === 0 ? "-100%" : "100%"; } },
                { x: "0%", scale: 1, duration: 0.4, stagger: 0.03, ease: "expo.ease" }
            ).fromTo(
                ".cardHeight",
                { y: function (idx) { return idx === 0 ? "-100%" : "100%"; }, scale: 0 },
                { y: "0%", scale: 1, duration: 0.4, stagger: 0.03, ease: "expo.ease" },
                "<"
            );
        };
    } else {
